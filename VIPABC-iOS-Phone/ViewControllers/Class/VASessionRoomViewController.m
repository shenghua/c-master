...
NSString *const _Nonnull VASessionRoomWillCloseNotification = @"VASessionRoomWillCloseNotification";

@interface VASessionRoomViewController () <WYPopoverControllerDelegate, LiveSessionDelegate, PopoverMenuDelegate>
...
@implementation VASessionRoomViewController

- (instancetype)init {
    
    UIStoryboard *sb = [UIStoryboard storyboardWithName:@"VASessionRoomViewController" bundle:nil];
    
    self = (VASessionRoomViewController *)[sb instantiateViewControllerWithIdentifier:@"VASessionRoomViewController"];
...
//    }
    ...
    // start by vipabc
    UITapGestureRecognizer *tapGesture = [[UITapGestureRecognizer alloc] initWithTarget:self action:@selector(showTutorial:)];
    self.helpLabel.userInteractionEnabled = YES;
    [self.helpLabel addGestureRecognizer:tapGesture];
...
- (IBAction)stopButtonPressed:(UIBarButtonItem *)sender {
    ...
    alert.rightButtonAction = ^(AlertView *alert) {
        ...
        [[NSNotificationCenter defaultCenter] postNotificationName:VASessionRoomWillCloseNotification object:nil];
    };
    [alert show];
}
...
- (void)showReloadAlert {
    __weak VASessionRoomViewController *weakSelf = self;
    dispatch_async(dispatch_get_main_queue(), ^{
...
//        self.videoView.frame = CGRectMake(480, 50, 100, 100);
