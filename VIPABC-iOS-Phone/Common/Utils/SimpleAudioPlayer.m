...
- (AVAudioPlayer *)playFileWithData:(NSData *)data volume:(CGFloat)vol loops:(NSInteger)loops withCompletionBlock:(CompletionBlock)completion
{
...
//    NSURL *fileURL = [NSURL fileURLWithPath:filePath isDirectory:NO];
    AVAudioPlayerWithCompletionBlock *player = [[AVAudioPlayerWithCompletionBlock alloc] initWithData:data error:&error];
    player.volume = vol;
...
- (AVAudioPlayer *)playFileWithData:(NSData *)data {
    
    return [self playFileWithData:data volume:1.0f loops:0 withCompletionBlock:nil];
}
...
+ (AVAudioPlayer *)playFileWithData:(NSData *)data {
    return [[SimpleAudioPlayer shared] playFileWithData:data];
}
