...
+ (SVProgressHUD*)sharedView {
    ...
    dispatch_once(&once, ^ {
        ...
        testActivityIndicator.center = CGPointMake(40, 40);
        testActivityIndicator.color = [UIColor redColor];
        [testActivityIndicator startAnimating];
        
        UIView *layerView = [[UIView alloc] initWithFrame:CGRectMake(0, 0, 80, 80)];
        layerView.center = loadingView.center;
        layerView.alpha = 0.8;
        layerView.backgroundColor = [UIColor whiteColor];
        layerView.layer.cornerRadius = 10;
        [layerView addSubview:testActivityIndicator];
...
+ (void)showErrorWithStatus:(NSString *)string {
    ...
    UIAlertView *alertView = [[UIAlertView alloc] initWithTitle:nil message:string delegate:self cancelButtonTitle:@"确定" otherButtonTitles:nil, nil];
    [alertView show];
//    [self sharedView];
