...
//#import <JSQMessagesViewController/JSQSystemSoundPlayer.h>
...
NSString const *kSessionRoleTypeStudent     = @"student";
NSString const *kSessionRoleTypeCohost      = @"cohost";
NSString const *kSessionRoleTypeCoordinator = @"coordinator";
NSString const *kSessionRoleTypeSales = @"sales";

@interface MessagesViewController ()
@property (nonatomic, strong, readwrite) NSSet *students;
...
- (void)didPressSendButton:(UIButton *)button withMessageText:(NSString *)text senderId:(NSString *)senderId senderDisplayName:(NSString *)senderDisplayName date:(NSDate *)date {
    ...
    [self.session sendMessageToAll:text];
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath {
    ...
    cell.textView.font = [UIFont montserratFontOfSize:14.f];
...
- (NSAttributedString *)collectionView:(JSQMessagesCollectionView *)collectionView attributedTextForMessageBubbleTopLabelAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (![msg.senderId isEqualToString:self.senderId] || [msg.senderDisplayName rangeOfString:@" to (null)"].location != NSNotFound) {
        
        // MAGIC: update display name with consultant name
        NSString *newDisplayName = msg.senderDisplayName;
        if ([msg.senderDisplayName rangeOfString:@" to (null)"].location != NSNotFound) {
            newDisplayName = [msg.senderDisplayName stringByReplacingOccurrencesOfString:@"(null)" withString:self.consultantName];
        }
        NSMutableAttributedString *sender = [[NSMutableAttributedString alloc] initWithString:newDisplayName attributes:@{NSFontAttributeName: [UIFont montserratFontOfSize:11.f]}];
