...
@property (nonatomic, weak) WbTriangle *weakSelf;
@end

@implementation WbTriangle
...
- (void)drawRect:(CGRect)rect {
    ...
    [path moveToPoint:CGPointMake(_lineSize / 2.0, _lineSize / 2.0 + abs(_height))];
    [path addLineToPoint:CGPointMake(_lineSize / 2.0 + abs(_width), _lineSize / 2.0 + abs(_height))];
    if (_type == 0)
        [path addLineToPoint:CGPointMake(_lineSize / 2.0 + abs(_width) / 2.0, _lineSize / 2.0)];
    else if (_type == 1)
        [path addLineToPoint:CGPointMake(_lineSize / 2.0, _lineSize / 2.0)];
    [path closePath];
