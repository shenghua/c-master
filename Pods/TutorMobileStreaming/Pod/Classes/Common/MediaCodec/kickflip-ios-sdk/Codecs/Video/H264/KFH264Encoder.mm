...
- (void) shutdown {
    [_encoder encodeWithBlock:nil onParams:nil];    // https://github.com/Kickflip/kickflip-ios-sdk/issues/7
    [_encoder shutdown];
    _encoder = nil;
}
...
- (instancetype) initWithBitrate:(NSUInteger)bitrate width:(int)width height:(int)height {
    if (self = [super initWithBitrate:bitrate]) {
        ...
        [_encoder encodeWithBlock:^int(NSArray* dataArray, CMTimeValue ptsValue) {
            ...
            if (self.delegate)
                [self.delegate encoder:self videoSpsAndPpsData:_videoSPSandPPS];
            return 0;
        }];
    }
    return self;
